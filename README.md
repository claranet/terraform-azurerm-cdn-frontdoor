...
| security\_policies | CDN FrontDoor Security policies configurations. | <pre>list(object({<br>    name                 = string<br>    custom_resource_name = optional(string)<br>    firewall = object({<br>      firewall_policy_name = string<br>      patterns_to_match    = optional(list(string), ["/*"])<br>      custom_domain_names  = optional(list(string), [])<br>      endpoint_names       = optional(list(string), [])<br>    })<br>  }))</pre> | `[]` | no |
